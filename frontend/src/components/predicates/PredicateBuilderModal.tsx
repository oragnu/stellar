...
export function PredicateBuilderModal({ open, onOpenChange, editing }: PredicateBuilderModalProps) {
  return (
    <Modal open={open} onOpenChange={onOpenChange} title={editing ? "Edit smart filter" : "New smart filter"}>
      {/* Keyed by the predicate being edited (or "new") and only mounted while open, so the
       * form always starts from fresh state instead of syncing itself via an effect. */}
      {open && <PredicateBuilderForm key={editing?.id ?? "new"} editing={editing} onOpenChange={onOpenChange} />}
    </Modal>
  );
}

interface PredicateBuilderFormProps {
  editing?: Predicate | null;
  onOpenChange: (open: boolean) => void;
}

function PredicateBuilderForm({ editing, onOpenChange }: PredicateBuilderFormProps) {
  const emptyBody = useEmptyPredicateBody();
  const [name, setName] = useState(editing?.name ?? "");
  const [body, setBody] = useState<PredicateGroupNode>(editing?.body ?? emptyBody);
